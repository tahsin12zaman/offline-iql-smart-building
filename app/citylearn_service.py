...
# This avoids requiring the CityLearn PyPI package during cloud deployment.
...
# Existing project evaluation utilities.
...
# These imports must happen after the paths above have been added.
...
def validate_files(algo_name, seed):
    model_path = (
        PROJECT_ROOT
        / "results"
        / f"citylearn_{algo_name.lower()}_seed_{seed}.d3"
    )

    missing = [
        str(path)
        for path in (DATA_PATH, SCHEMA_PATH, model_path)
        if not path.exists()
    ]
...
def run_live_evaluation(algo_name, seed, max_steps=MAX_STEPS):
    ...
    try:
        ...
        for step in range(max_steps):
            ...
            action = np.clip(
                algo.predict(x)[0].astype(np.float32),
                low,
                high,
            )
            ...
            # CityLearn increments time_step after env.step().
            # Therefore use time_step - 1 to retrieve values associated
            # with the transition that has just completed.
