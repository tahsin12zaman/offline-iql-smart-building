...
def build_and_load_algo(algo_name, model_path, dataset, observations, actions):
...
def evaluate_policy(algo, schema_path, max_steps):
    ...
    for _ in range(max_steps):
        action = algo.predict(obs.reshape(1, -1))[0].astype(np.float32)
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/citylearn_eval_scores.csv")
    ...
    dataset, observations, actions = load_dataset(args.data)

    rows = []

    for algo_name in ["bc", "iql", "cql"]:
        model_path = Path(f"results/citylearn_{algo_name}_seed_{args.seed}.d3")
        ...
        print(f"{algo_name.upper()} score:", score)
        print("Steps:", steps)
    ...
    with open(out_path, "a", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["algorithm", "seed", "score", "steps", "max_steps"],
        )
    ...
    print("=" * 70)
    print("Saved evaluation scores to:", out_path)
    print("=" * 70)
